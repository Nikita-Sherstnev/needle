...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        if -1 in new_shape:
            calc_dim = prod(self.shape) / -prod(new_shape)
            new_shape = list(new_shape)
            for i, dim in enumerate(new_shape):
                if dim == -1:
                    new_shape[i] = int(calc_dim)
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        new_shape = [self.shape[axe] for axe in new_axes]
        strides = tuple([self.strides[axe] for axe in new_axes])
        return self.as_strided(new_shape, strides)
        ### END YOUR SOLUTION

    def swap_axes(self, a1: int, a2: int):
        new_shape = list(self.shape)
        new_strides = list(self.strides)

        new_shape[a1], new_shape[a2] = self.shape[a2], self.shape[a1]
        new_strides[a1], new_strides[a2] = self.strides[a2], self.strides[a1]
    ...
    def __getitem__(self, idxs):
        ...
        return self.make(shape, strides=tuple(strides), device=self._device, handle=self._handle, offset=offset)
        ### END YOUR SOLUTION
    ...
    def flip(self, axes):
        ...
        for a in axes:
            new_strides[a] *= -1
            offset = x.shape[a] - 1
            for b in range(1, len(x.shape) - a):
                offset *= x.shape[a + b]
            new_offset += offset
        out = NDArray.make(
    ...
    def pad(self, axes):
        ...
        out = self.device.full(shape, 0.)
        out[idx] = self.compact()
