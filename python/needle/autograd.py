...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        for ind, input_node in enumerate(node.inputs):
            v_k = node.op.gradient_as_tuple(node.grad, node)[ind].detach()
            if input_node in node_to_output_grads_list:
                node_to_output_grads_list[input_node].append(v_k)
            else:
                node_to_output_grads_list[input_node] = [v_k]
    ### END YOUR SOLUTION
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    topo_sort_dfs(node_list[0], visited)
    return visited
    ### END YOUR SOLUTION
